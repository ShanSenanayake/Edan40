...
> generateMusicChord range key prog = line $ map chordToMusic $ zip (pickBestChords key range chords) durs
...
> generateCandidates range key c = getChordPatterns (generateChordRange range (getBasicTriad key c))
